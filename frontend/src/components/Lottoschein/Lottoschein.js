...
return (
  <div className="flex flex-col items-center min-h-screen">
    <div id="lottschein" className="flag grid grid-cols-7 gap-2 border-red-500 p-1 bg-red-100 border-[1.5px] hover-stift relative z-0">
      {numbers.map((row, i) => (
        row.map((num, j) => (
          <div key={`${i}-${j}`} className="group relative border bg-white border-red-500 text-red-500 p-1 flex items-center justify-center z-[-1]">
            {num}
